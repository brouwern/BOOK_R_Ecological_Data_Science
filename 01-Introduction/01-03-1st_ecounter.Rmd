...
* Use can use R directly, but most people use a GUI of some kind
...
Most people who use R do not actually use the program itself - they use a GUI (graphical user interface) "front end" that make R a bit easier to use.  You will probably run into the icon for the underlying R program on your desktop or elsewhere on your computer.  It usually looks like this:
...
When you open up you'll be greated by a fairly busy array of menus and thingys.  A typical fresh starting point in RStudio is shown in Figure 1.

```{r echo=FALSE , fig.cap = "RStudio when first opened."}
knitr::include_graphics("images/RStudio_1st_view-800x600.png")
```

When referring to RStudio, there are two terms that need to be understood.  As shown in Figure 2, there is the **console** section of RStudio and the **script editor** or **source viewer**.  
