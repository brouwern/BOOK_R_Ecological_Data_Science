...
R comes with a number of datasets ready to use.  A famous dataset frequently used in statistics is a set of measurements made on three species of irises and used to demonstate some statistical principles by genetistics and statistician R.A. Fisher.

We can put the iris dataset into R's workig memoring using the data() command
```{r}
data(iris)
```
...
We can see that there are five rows of data. Three contain information about the length and width of the parts of the flower (Sepals and Petals) and teh last holds the names of the species.
...
Base R however is surrounded by a universe of extensions built by statistician, programmers, academics and businesses that use R for analyses.  Some of these are fairly standard and are downloaded along with base R and just need to be explicilty installed.  Other have to be downloaded from the internet and installed.  Most packages contain data in order to demonstate what they do.

### Loading a package contained in base R

One package that is automatically downloaded but not automatically *installed* with base R is the "MASS" package, which stands for "Modern Applied Statistics in R"; S is the software that preceded R.  We can install this package and make it functinally using the library() command

```{r}
library(MASS)
```

The MASS package has a biological dataset called "crabs" that you can put into working memory using data(crabs).  We can then look at it using head(),Viwe(), tail(), summary(), etc.  We can find out more about the dataset using the help file, accssed via ?crabs
...
We can plot the relationshp between the FL and RW variables using a scatterplot.
```{r}
...
When data is being worked with in R, it lives in a place called the **workspace.**  The workspace is not immediately transparent to you while working in R.  It lives behind the scenes in what is essentiall R's working memore.  We can see what's on R's mind using the ls() command
...
We can also leanr more about is using the is() command
```{r}
is(my.mean)
```
...
R objects can be just about anyting.  We can assign letter to an R object like this
```{r}
my.abc <- c("a","b","c")
```
Note that we have teh letter each surrounded by quotes, and all 3 of them within c(...)
...
Several things get spit out, but the first one is importnat: "data.frame"  Dataframes are fundamental units of analysis in R.  Most of the data you will load into R and work within R will be in a dataframe.
...
Note that the varibles "sp", which stands for "Species", and "sex" are follwed by the word "Factor."  A **factor variable** is something that is or is summarized as discrete categories.  For the species factor, there are two levels: the "B" species and the "O" species.


## Load data from an external R package

Many packages have to be explicilty downloaded and installed in order to use their functions and datasets.  Note that this is a **two step process**:
...
Well download a package used for plotting called ggplot2, which stands for "Grammer of graphics"

```{r eval = FALSE}
install.packages("ggplot2")
```

Often when you download a package you'll see a fair bit of red text.  Usually there's nothign of interest hear, but sometimes you need to read over it for hints about why somethign didn't work.

### Step 2: Explicitly loading a package

THe install.packages() functions just saves teh package software to R; now you need to tell R "I want to work with the package".  This is done usig the library() function. (Its called library because another name for packages is libraries)

```{r}
library(ggplot2)
```

ggplot2 has a dataset called "msleep" which has information on the relationship between the typical size of a species and its brain weight, among other thigns

We load the data actively into R's memore using data(), and can look at the colum names using names()
```{r}
data(msleep)
names(msleep)
```

We can now explore this data set as before using sumamry(), str(), etc.

Anoter useful command when you are working with a new dataset is dim().  THis tells you the dimension of the dataframe
...
The animals in this data vary in size from mice to elephants and so a lot of the datapoints are scrunched together.  A trick to make this easier to see is to take teh log of the brainwt and bodywt variable.  In R, we can do this on the fly like this using the log() command
...
So far we have only looked at dataset that are already formatted into **dataframe** by somebody for us.  Now we want to look at how to set up datasets ourselves.  When datasets are small its possible to enter them more or less directly into R by typing out all of the numbers in a script.  This only works well for when datasets are small; even when datasets are small its best to keep them seperate from your R code in a spreadsheet file.  However, its useful to know how to load data this way; even when an exercise in this book loads data from a package or spreadsheet I will also often include the code to load it directly just in case there is an issue with download the package or file.

### The eagles have landed - in your R workspace

In a subsequent exercise we will practice using data on the number of eagles in Pennsylvania and other states in teh USA.  We can load this data into R by making R objects, and then turning these objects into a dataframe. 
...
We can install ggpubr using install.pacakges().  Note that the name of the package, ggpubr, is in quotes.
```{r eval = FALSE}
...
Occassionally you might try to load a package and it won't automatically install or download the dependency, usually because its not yet downloaded.  If this happend with magrittr we would just have to download it using "install.packages("magrittr")".
